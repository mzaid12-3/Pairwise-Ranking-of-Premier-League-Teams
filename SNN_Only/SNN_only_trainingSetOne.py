...
class SiameseHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        print(f"Input layer size (Flattened): {self.input_shape}")

        x = Flatten()(input)
        for i in range(hp.Int('num_layers', 1, 3)):
            units = hp.Int('units', min_value=10, max_value=70, step=3)
            x = Dense(units=units, activation='relu')(x)
            print(f"Hidden layer {i + 1} size: {units}")
            dropout = hp.Float('dropout', min_value=0.005, max_value=0.3, step=0.01)
            x = Dropout(rate=dropout)(x)
        # Output layer for the base network
        x = Dense(5, activation='relu')(x)
        print("Output size of the base network (before distance calculation): 5")
        ...
        # Output size of the Siamese Network is 1 because it outputs a distance
        print("Output size of the Siamese Network (distance): 1")
...
best_model.save('SNN_trainingSetOne.keras')
