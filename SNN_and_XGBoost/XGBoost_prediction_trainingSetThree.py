...
from creatingPairsForTrainingSetThree_SNNXGBoost import trainingSetOne
...
with open('SNN_XGBoost_similarity_scores_trainingSetThree.txt', 'r') as f:
    similarity_scoresTrainingSetOne = [float(line.strip()[1:-1]) for line in f.readlines()]


bst = xgb.Booster()
bst.load_model('SNN_XGBoosttrainingSetThree.xgb')
...
with open("predicted_ranks_trainingSetThree.txt", "w") as file:
