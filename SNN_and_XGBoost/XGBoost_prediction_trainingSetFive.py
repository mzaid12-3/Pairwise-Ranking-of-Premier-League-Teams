...
if __name__ == "__main__":
    ...
    with open('SNN_XGBoost_similarity_scores_trainingSetFive.txt', 'r') as f:
        similarity_scores  = [float(line.strip()[1:-1]) for line in f.readlines()]
    ...
    for (team_a, team_b), score in zip(combined_team_pairs, similarity_scores):
        # For team_a
        team_scores[team_a] = team_scores.get(team_a, 0) + score
        team_counts[team_a] = team_counts.get(team_a, 0) + 1

        # For team_b
        team_scores[team_b] = team_scores.get(team_b, 0) + score
        team_counts[team_b] = team_counts.get(team_b, 0) + 1

    # Calculate mean scores for each team
    mean_scores = {team: total_score / team_counts[team] for team, total_score in team_scores.items()}

    # Convert the dictionary to a dataframe
    mean_scores_df = pd.DataFrame(list(mean_scores.items()), columns=['team identifier', 'mean similarity score'])

    # Merge mean_scores_df with aggregated_data
    aggregated_data = pd.merge(aggregated_data, mean_scores_df, on="team identifier", how="left")
    print("aggregated_data: ", aggregated_data)
    # Load the best model
    bst = load_best_model('SNN_XGBoosttrainingSetFive.xgb')
