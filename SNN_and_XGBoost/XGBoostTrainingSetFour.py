...
x_last_epoch = np.round(np.loadtxt(f'activations_a_epoch_trainingSetFour49.txt'),1)
y_last_epoch = np.round(np.loadtxt(f'activations_a_epoch_trainingSetFour49.txt'),1)
...
bst = load_best_model('XGBoosttrainingSetFour.xgb')
...
season_weights = [1, 2, 3, 4, 5, 6, 7,8, 9 ]
weighted_standings = []
for team_id in actual_rankings:
    team_weighted_standing = sum(
        season_standings[season].get(team_id, len(actual_rankings) + 1) * season_weights[season] for season in range(9))
...
with open('predicted_rankingsTrainingSetFour.txt', 'w') as f:
