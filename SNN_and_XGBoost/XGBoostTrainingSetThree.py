...
x_last_epoch = np.round(np.loadtxt(f'activations_a_epoch_trainingSetThree49.txt'),1)
y_last_epoch = np.round(np.loadtxt(f'activations_a_epoch_trainingSetThree49.txt'),1)
...
season_weights = [1, 2, 3, 4, 5, 6, 7,8]
weighted_standings = []
for team_id in actual_rankings:
    team_weighted_standing = sum(
        season_standings[season].get(team_id, len(actual_rankings) + 1) * season_weights[season] for season in range(8))
