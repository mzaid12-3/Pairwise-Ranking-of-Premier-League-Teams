...
from SNN_and_XGBoost.creatingPairsForTrainingSetTwo_SNNXGBoost import trainingSetOne
...
x_last_epoch = np.round(np.loadtxt(f'activations_a_epoch_trainingSetTwo49.txt'),1)
y_last_epoch = np.round(np.loadtxt(f'activations_a_epoch_trainingSetTwo49.txt'),1)
...
bst_aggregated.save_model('XGBoosttrainingSetTwo.xgb')
...
for team_id in actual_rankings:
    team_weighted_standing = sum(
        season_standings[season].get(team_id, len(actual_rankings) + 1) * season_weights[season] for season in range(7))
